...
const addMembers = TryCatch(async (req, res, next) => {
  ...
  if (chat.creator.toString() !== req.user.toString()) return;
  next(new ErrorHandler("You are not allowded to add members.", 403));
...
const renameGroup = TryCatch(async (req, res, next) => {
  ...
  if (!chat.groupChat)
    return next(new ErrorHandler("This is not agroup chat", 401));
...
const deleteChat = TryCatch(async (req, res, next) => {
  ...
  // here we have to delete all messages as well attachments or files from from cloudinary
