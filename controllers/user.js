...
const newUser = TryCatch(async (req, res, next) => {
  ...
  if(!file) return next( new ErrorHandler("Pleasw Upload Avatar"))
...
const login = TryCatch(async (req, res, next) => {
  ...
  if (!isMatch) return next(new ErrorHandler("password didn't match", 404));
...
// flat le single array ma data leuxa
